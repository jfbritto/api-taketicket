<x-layouts.app title="Ticket — {{ $ticket->ticket_code }}">
    ...
    <div class="max-w-2xl mx-auto px-4 py-8">
        <div class="mb-6">
            <a href="{{ route('my-tickets') }}" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">&larr; Back to My Tickets</a>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border overflow-hidden">
            ...
            <div class="px-6 pt-5 pb-2 flex items-center justify-between">
                ...
                @if($ticket->status === \App\Enums\TicketStatus::VALID)
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-green-100 text-green-800">Valid</span>
                @elseif($ticket->status === \App\Enums\TicketStatus::USED)
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-gray-100 text-gray-800">Used</span>
                @else
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-red-100 text-red-800">Cancelled</span>
                @endif
            </div>
            ...
            <div class="px-6 py-4 space-y-4">
                ...
                {{-- Ticket Code --}}
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500 font-medium">Ticket Code</span>
                    <span class="text-gray-800 font-mono">{{ $ticket->ticket_code }}</span>
                </div>

                @if($ticket->participant)
                <div class="border-t pt-4">
                    <h2 class="text-sm font-semibold text-gray-700 mb-3">Participant Information</h2>
                ...
                <div class="border-t pt-4 flex flex-col items-center">
                    <p class="text-sm text-gray-500 mb-3">Scan at check-in</p>
